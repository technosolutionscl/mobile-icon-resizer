...
function convertiOS(options, callback) {
  ...
  var json = JSON.stringify(options.config.iOSIcons);
  fs.writeFile(path.join(options.iosOutputFolder, 'Content.json'), json, 'utf8');
}
...
function convertLaunchImages(options, callback) {
  ...
  var json = JSON.stringify(options.config.iOSLaunchImages);
  fs.writeFile(path.join(options.iosLaunchImageFolder, 'Content.json'), json, 'utf8');
}
...
var resize = function (options, callback) {
  ...
  options.bg = options.bg || defaults.BACKGROUND_COLOR;
