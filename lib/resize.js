...
function executeResize(options, callback) {
    var
    filename    = options.platform+options.outputFile.split('/'+options.platform).pop(), //name used for logging
    ...
    command+=' -thumbnail ' + dimensions + ' -gravity center -extent ' + dimensions + ' "' + options.inputFile + '" "' + options.outputFile + '"';
...
var checkImageType=function(options,cback){
    ...
    magic.detectFile(options.originalIconFilename, function(err, result) {
        ...
        if(result=='image/svg+xml'){
            var newname=options.originalIconFilename.replace(/\.svg$/i,'')+'.png';
            executeResize(
                {
                    platform    : 'none',
                    inputFile   : options.originalIconFilename,
