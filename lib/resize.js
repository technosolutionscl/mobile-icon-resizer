...
function convertAndroid(options, callback) {
    ...
    function handleImage(image, done) {
        ...
        mkdirp.sync(baseFolder);
        var fileName = (image.filename)?path.join(baseFolder, image.filename):path.join(baseFolder, options.androidOutputFilename);
        executeResize(
            {
                platform  : 'android',
                inputFile : options.originalIconFilename,
